...
export const httpApi: ApiInterface = {
  ...
  async doAfk(endedAt?: number | null): Promise<AfkTransitionResult> {
    const { data } = await (await withClientId()).post<AfkTransitionResult>('/api/afk', endedAt == null ? {} : { endedAt })
    return data
  },
  ...
  async submitDayScriptProgress(date: string, body?: { focusActivity?: DayScriptFocusActivity[] }): Promise<SubmitDayScriptProgressResult> {
  ...
  async rescheduleDayScriptFocus(date: string, body: { expectedRevision: number; sortOrders: number[] }): Promise<{ script: DayScriptDocument; changed: boolean }> {
    const { data } = await (await withClientId()).post(`/api/day-scripts/${encodeURIComponent(date)}/reschedule-focus`, body)
    return data
  },

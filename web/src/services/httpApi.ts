...
export const httpApi: ApiInterface = {
  ...
  async createTask(req: CreateTaskRequest): Promise<Task> {
    const { data } = await client.post<Task>('/api/tasks', req)
    return data
  },

  async updateTask(id: string, req: UpdateTaskRequest): Promise<Task | null> {
    const { data } = await client.put<Task>(`/api/tasks/${id}`, req)
    return data
  },
  ...
  async markTaskDone(id: string): Promise<Task | null> {
    const { data } = await client.put<Task>(`/api/tasks/${id}/done`)
    return data
  },

  async fetchTaskEntries(taskId: string): Promise<TaskEntry[]> {
    const { data } = await client.get<TaskEntry[]>(`/api/tasks/${taskId}/logs`)
    return data
  },

  async submitTaskEntry(taskId: string, content: string, type?: 'body' | 'log'): Promise<TaskEntry> {
    const { data } = await client.post<TaskEntry>(`/api/tasks/${taskId}/logs`, { content, type })
    return data
  },

  async updateTaskEntry(taskId: string, entryId: string, content: string): Promise<TaskEntry | null> {
    const { data } = await client.put<TaskEntry>(`/api/tasks/${taskId}/logs/${entryId}`, { content })
    return data
  },

  async takeOverTask(taskId: string): Promise<WorkSession> {
    const { data } = await client.post<WorkSession>(`/api/tasks/${taskId}/takeover`)
    return data
  },

  async doAfk(): Promise<void> {
    await client.post('/api/afk')
  },

  async getCurrentSession(): Promise<WorkSession | null> {
    const { data } = await client.get<WorkSession | null>('/api/sessions/current')
    return data
  },

  async fetchSessions(start: number, end: number): Promise<WorkSession[]> {
    const { data } = await client.get<WorkSession[]>('/api/sessions', { params: { start, end } })
    return data
  },

  async dropTask(taskId: string, reason: string): Promise<Task | null> {
    const { data } = await client.post<Task>(`/api/tasks/${taskId}/drop`, { reason })
    return data
  },

  async fetchTodayTasks(): Promise<Task[]> {
    const { data } = await client.get<Task[]>('/api/tasks/today')
    return data
  },
  ...
  async fetchTodayReport(): Promise<{
    ...
    const { data } = await client.get('/api/reports/today')
    return data
  },
  ...
  async fetchSummary(): Promise<{
    ...
    const { data } = await client.get('/api/reports/summary')
    return data
  },
}

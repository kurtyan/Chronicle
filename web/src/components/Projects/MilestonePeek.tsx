...
import { useProjectLabels } from './common'
...
export function MilestonePeek({ target, periodLabel, onEnter, onLeave }: { target: MilestonePeekTarget; periodLabel: string; onEnter: () => void; onLeave: () => void }) {
  ...
  const schedule = milestoneSchedule(m, locale)
  return createPortal(<div ref={panel} id="milestone-peek" role="tooltip" data-testid="milestone-peek" onMouseEnter={onEnter} onMouseLeave={onLeave} className="fixed z-[60] max-h-[calc(100vh-24px)] w-[310px] max-w-[calc(100vw-24px)] overflow-y-auto rounded-xl border border-border/70 bg-popover p-4 text-popover-foreground shadow-xl" style={{ left: position.left, top: position.top, visibility: position.ready ? 'visible' : 'hidden' }}>
    <p className="text-[11px] text-muted-foreground">{m.areaName} <span className="mx-1">/</span> {statusLabels[m.status]}{m.reviewStatus === 'pending' ? t('project.details.reviewPending') : ''}</p>
    ...
    <div className="mt-3 flex gap-6 border-t border-border/60 pt-3 text-xs">{periodLabel !== t('project.overview.totalShort') && <div><p className="text-[10px] text-muted-foreground">{t('project.details.periodTime', { period: periodLabel })}</p><p className="mt-1 font-medium">{duration(m.periodMs)}</p></div>}<div><p className="text-[10px] text-muted-foreground">{t('project.details.totalTime')}</p><p className="mt-1 font-medium">{duration(m.totalMs)}</p></div></div>
    {m.latestProgress && <p className="mt-3 line-clamp-3 text-xs leading-5"><span className="text-muted-foreground">{t('project.details.progressPrefix')} </span>{m.latestProgress}</p>}
    {m.blockers && <p className="mt-2 line-clamp-2 text-xs leading-5 text-amber-700 dark:text-amber-400">{t('project.details.blockersPrefix')} {m.blockers}</p>}
    <p className="mt-3 text-[10px] text-muted-foreground">{t('project.details.peekHint')}</p>
  </div>, document.body)
}

...
function findPublicDir(): string {
  ...
  const candidates = ['./public', '../public', './server/public']
...
export const app = new Hono<{ Variables: { clientId: string } }>()
...
function validateTaskPayload(body: any, create = false): string | null {
  ...
  if (body.primaryMilestoneId !== undefined && body.primaryMilestoneId !== null && typeof body.primaryMilestoneId !== 'string') return 'primaryMilestoneId must be a string or null'
  if (body.expectedProjectRevision !== undefined && (!Number.isInteger(body.expectedProjectRevision) || body.expectedProjectRevision < 0)) return 'Invalid expectedProjectRevision'
  if (body.assignmentToken !== undefined && typeof body.assignmentToken !== 'string') return 'Invalid assignmentToken'
  return null
}
...
// Project routes share the same origin and database-maintenance middleware.
...
app.post('/api/tasks', async (c) => {
  ...
  try {
    ...
    if ([400, 404, 409, 422].includes(err?.status)) return c.json({ error: message, code: err.code }, err.status)
...
app.put('/api/tasks/:id', async (c) => {
  ...
  let task
  try {
    task = await service.updateTask(c.req.param('id'), body)
  } catch (err: any) {
    if ([400, 404, 409, 422].includes(err?.status)) return c.json({ error: err.message, code: err.code }, err.status)
    throw err
  }
  if (!task) return c.json({ error: 'Not found' }, 404)
...
app.get('/api/notes', async (c) => {
  ...
  try {
    const rawFilters = c.req.query('projectFilters')
    const projectFilters = rawFilters === undefined ? undefined : JSON.parse(rawFilters)
    return c.json(await service.getNotes({ includeArchived, query, limit, projectFilters }))
  } catch (error: any) { return c.json({ error: error?.message || 'Invalid Note filters' }, 400) }
})
...
// Initialization is separate from binding sockets so the same HTTP routes can
// be verified in-process against an isolated SQLite database.
export function initializeDatabaseState(): void {
  initDb()

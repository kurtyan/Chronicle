...
Remove `target_root/.claude/commands/chronicle/` if it exists (legacy location from old installations).

For each entry in `manifest.json.skills`:
1. Read `source_root/<file>` (the `file` field from manifest, relative to `skills/`)
2. Write the content to `target_root/.claude/commands/<file>`
...
Confirm all skill files from the manifest are present in `target_root/.claude/commands/` and report: "Installed Chronicle skills v{version}: {list of skill names}."

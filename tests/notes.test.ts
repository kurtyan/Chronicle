...
test.describe('Notes', () => {
  ...
  test('Notes page creates and autosaves a note', async ({ page }) => {
    ...
    await page.getByRole('combobox', { name: '搜索 Notes 或输入 # 标签', exact: true }).fill(`UiNote-${unique}`)
  ...
  test('Notes page searches body content through the API', async ({ page }) => {
    ...
    await page.getByRole('combobox', { name: '搜索 Notes 或输入 # 标签', exact: true }).fill(`NeedleBody-${unique}`)
  ...
  test('archived notes can be shown, searched, opened, and unarchived', async ({ page }) => {
    ...
    await page.getByRole('combobox', { name: '搜索 Notes 或输入 # 标签', exact: true }).fill(`ArchiveBody-${unique}`)

...
test.describe('Phase 4: Task Status Enhancement', () => {
  test('T1: Task status flow - PENDING -> DOING -> DONE -> DOING -> DROPPED', async ({ page }) => {
    ...
    await page.request.put(`/api/tasks/${task.id}`, { data: { status: 'DOING' } })
  ...
  test('T2: Filter by Done and Dropped', async ({ page }) => {
    ...
    const task = await res.json()
    await page.request.put(`/api/tasks/${task.id}`, { data: { status: 'DOING' } })
    await page.request.put(`/api/tasks/${task.id}/done`)
    ...
    await page.request.post(`/api/tasks/${task2.id}/drop`, { data: { reason: '废弃测试' } })
  ...
  test('T3: Take Over / AFK workflow', async ({ page }) => {
    ...
    const sessionsRes = await page.request.get('/api/sessions?start=0&end=9999999999999')
  ...
  test('T6: Drop dialog - confirm disabled without reason', async ({ page }) => {
    ...
    const confirmButton = page.getByRole('button', { name: '废弃' }).last()

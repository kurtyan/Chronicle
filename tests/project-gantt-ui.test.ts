...
async function filterArea(page: import('@playwright/test').Page, name: string) {
  await page.getByRole('button', { name: '方向筛选', exact: true }).click()
  const picker = page.locator('[data-project-select-popup]')
  await picker.getByRole('combobox').fill(name)
  await picker.getByRole('option').filter({ hasText: name }).click()
}
...
test('Gantt uses explicit schedules, distinguishes undated, target-only and open-ended milestones, and opens inherited Notes', async ({ page, request }) => {
  ...
  await filterArea(page, area.name)
  ...
  await expect(undated.getByRole('button', { name: '安排日期', exact: true })).toBeVisible()
  const ongoing = chart.getByTestId('gantt-milestone-row').filter({ hasText: milestones[2].name })
  await expect(ongoing.getByRole('button', { name: /持续跟踪, 关联笔记/ })).toBeVisible()
  ...
  await expect(targetOnly.getByRole('button', { name: /开始待定, 关联笔记/ })).toBeVisible()
  ...
  await panel.getByRole('button', { name: '更新进展', exact: true }).click()
  const summaryEditor = panel.getByTestId('project-inline-editor')
  await summaryEditor.getByRole('textbox', { name: '下一步', exact: true }).fill('验证一条完整使用路径，并记录结果')
  await summaryEditor.getByRole('button', { name: '保存', exact: true }).click()
  await expect(summaryEditor).toHaveCount(0)
  await expect(panel).toContainText('验证一条完整使用路径，并记录结果')
  const linkedNotes = panel.getByTestId('project-object-notes')
  await expect(linkedNotes.getByRole('heading', { name: '关联 Notes 1', exact: true })).toBeVisible()
  await expect(linkedNotes.getByRole('button', { name: note.title, exact: false })).toHaveCount(1)
  await panel.getByRole('button', { name: '关闭详情', exact: true }).click()
  await chart.getByRole('button', { name: new RegExp(`${milestones[0].name},.*关联笔记`) }).click()
  await expect(linkedNotes.getByRole('button', { name: note.title, exact: false })).toBeVisible()
  await linkedNotes.getByRole('button', { name: '在 Notes 中查看', exact: true }).click()
...
test('Gantt schedule edit persists across reload, rejects reversed dates, and leaves status and effort unchanged', async ({ page, request }) => {
  ...
  await row.getByRole('button', { name: '安排日期', exact: true }).click()
  const dialog = page.getByRole('dialog')
  await expect(dialog.getByRole('heading', { name: '调整排期', exact: true })).toBeVisible()
  await expect(dialog.getByRole('textbox', { name: '名称', exact: true })).toHaveCount(0)
  await expect(dialog.locator('textarea')).toHaveCount(0)
  await expect(dialog.getByRole('button', { name: '状态', exact: true })).toHaveCount(0)
  ...
  await expect(row.getByRole('button', { name: /关联笔记/ })).toBeVisible()
  await page.reload()
  await expect(page.getByTestId('gantt-milestone-row').filter({ hasText: milestone.name }).getByRole('button', { name: /关联笔记/ })).toBeVisible()
...
test('Gantt can navigate outside the current window and collapse a direction lane without losing its summary', async ({ page, request }) => {
  ...
  await page.getByTestId('project-gantt').getByRole('button', { name: /^窗口外\s+2$/ }).click()
  await expect(pointRow.getByRole('button', { name: /定位到里程碑/ })).toBeVisible()
  await expect(pointRow.getByRole('button', { name: /开始待定, 关联笔记/ })).toHaveCount(0)
  await row.getByRole('button', { name: /定位到里程碑/ }).click()
  await expect(row.getByRole('button', { name: /关联笔记/ })).toBeVisible()
...
test('Gantt keeps inclusive date boundaries, target-only points and ongoing milestones from occupying the same track', async ({ page, request }) => {
  ...
  await expect(rows[2].getByRole('button', { name: /开始待定, 关联笔记/ })).toBeVisible()
  await expect(rows[3].getByRole('button', { name: /持续跟踪, 关联笔记/ })).toBeVisible()
})
...
test('Gantt recomputes track allocation when schedule editing creates an overlap and retains it after reload', async ({ page, request }) => {
  ...
  await laterRow.getByRole('button', { name: /关联笔记/ }).click()
  await page.getByTestId('project-context-panel').getByRole('button', { name: '调整排期', exact: true }).click()
  const dialog = page.getByRole('dialog')
...
test('Gantt keeps long titles in exact short date bars without adding tracks or increasing lane height', async ({ page, request }) => {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const bar = rows[i].getByRole('button', { name: /关联笔记/ })
    await bar.hover()
  ...
  await expect(page.getByTestId('project-context-panel')).toContainText(area.nextStep)
  await page.getByTestId('project-context-panel').getByRole('button', { name: '关闭详情', exact: true }).click()
  await chart.getByRole('button', { name: '按月', exact: true }).click()
  await expect(chart.getByTestId('gantt-axis')).toContainText('月')
  for (let i = 0; i < rows.length; i++) {
    const markLocator = rows[i].getByTestId('gantt-time-mark')
    const mark = await markLocator.boundingBox()
    const hitTarget = await rows[i].getByRole('button', { name: /关联笔记/ }).boundingBox()
    expect(mark).not.toBeNull()
    expect(hitTarget).not.toBeNull()
    expect(mark!.width).toBeGreaterThan(0)
    expect(mark!.width).toBeLessThan(10)
    expect(hitTarget!.width).toBeGreaterThanOrEqual(mark!.width)
    // Click the actual painted mark, not its larger parent button. Closing the
    // nonmodal inspector changes the axis width; let Playwright wait for that
    // mark to stop moving instead of clicking a cached pre-resize coordinate.
    // A neighbouring transparent hit target still causes this action to fail.
    await markLocator.click()
    const panel = page.getByTestId('project-context-panel')
...
test('Gantt reveals milestone details on hover or focus, dismisses with Escape, and opens Notes without changing them', async ({ page, request }) => {
  ...
  await filterArea(page, area.name)
  const row = page.getByTestId('gantt-milestone-row').filter({ hasText: milestone.name })
  const bar = row.getByRole('button', { name: /关联笔记/ })
  ...
  await expect(peek).toContainText(new Date(milestone.startDate).toLocaleDateString('zh-CN', { year: 'numeric', month: 'short', day: 'numeric' }))
  await expect(peek).toContainText(new Date(milestone.targetDate).toLocaleDateString('zh-CN', { year: 'numeric', month: 'short', day: 'numeric' }))
  ...
  const panel = page.getByTestId('project-context-panel')
  await expect(peek).toHaveCount(0)
  await expect(panel.getByTestId('project-object-notes').getByRole('button', { name: note.title, exact: false })).toBeVisible()
  await panel.getByTestId('project-object-notes').getByRole('button', { name: note.title, exact: false }).click()
  ...
  await page.getByTestId('project-review-context').getByRole('button', { name: '返回项目', exact: true }).click()
...
test('Gantt shows cumulative effort once and explains a status filter with no matching milestones', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: '统计期间', exact: true }).click()
  await page.locator('[data-project-select-popup]').getByRole('option', { name: '全部时间', exact: true }).click()
  expect((await allPeriod).ok()).toBe(true)
  await row.getByRole('button', { name: /关联笔记/ }).click()
  const panel = page.getByTestId('project-context-panel')
  const effort = panel.getByTestId('project-object-effort')
  await expect(effort.getByRole('heading', { name: '累计的实际投入', exact: true })).toHaveCount(1)
  await expect(effort.getByText('0 分钟', { exact: true })).toHaveCount(1)
  await expect(effort.getByText('本期 0 分钟 · 0 个任务', { exact: true })).toBeVisible()
  await panel.getByRole('button', { name: '关闭详情', exact: true }).click()

  await page.getByRole('button', { name: '筛选', exact: true }).click()
  await page.getByRole('button', { name: '里程碑状态筛选', exact: true }).click()
  await page.locator('[data-project-select-popup]').getByRole('option', { name: '已完成', exact: true }).click()
  await expect(row).toHaveCount(0)
  await expect(chart.getByTestId('gantt-area-lane')).toHaveCount(1)
  await expect(chart.getByRole('button', { name: area.name, exact: true })).toBeVisible()
  await expect(chart.getByText('还没有里程碑', { exact: true })).toBeVisible()
  await page.getByRole('button', { name: '重置', exact: true }).click()
  await expect(row).toBeVisible()
  await expect(chart.getByText('还没有里程碑', { exact: true })).toHaveCount(0)
})
